...
void *_g2dSetVertex(void *vp, int i, float vx, float vy) {
    ...
    if (rctx.use_vert_color) { *(vp_color++) = OBJ_I.color; }
...
void _g2dEndLines() {
    ...
    if (rctx.use_strip) {
        ...
        for (i = 1; i < rctx.n; i += 1) { vi = _g2dSetVertex(vi, i, 0.f, 0.f); }
    } else {
...
void _g2dEndPoints() {
    ...
    // Build the vertex list
    for (i = 0; i < rctx.n; i += 1) { vi = _g2dSetVertex(vi, i, 0.f, 0.f); }
...
void g2dEnd() {
    ...
    if (rctx.use_z) sceGuEnable(GU_DEPTH_TEST);
    else
        sceGuDisable(GU_DEPTH_TEST);

    if (rctx.use_vert_color) sceGuColor(WHITE);
    else
        sceGuColor(rctx.cur_obj.color);

    if (rctx.tex == NULL) sceGuDisable(GU_TEXTURE_2D);
    else {
...
#ifdef USE_PNG
    if (strstr(path, ".png")) { tex = _g2dTexLoadPNG(fp); }
#endif
